...
async function downloadAsset(imageUrl, localPath) {
  return new Promise((resolve, reject) => {
    ...
    const fullUrl = `https://thoriummod.wiki.gg${imageUrl}`;
    ...
    const request = protocol.get(fullUrl, options, (response) => {
      if (response.statusCode === 200) {
        ...
        downloadAsset(
          redirectUrl.replace("https://thoriummod.wiki.gg", ""),
          localPath
        )
          .then(resolve)
          .catch(reject);
...
const stageMapping = {
  ...
  "post-mech-boss": {
    title: "Post-Mechanical Boss",
    icon: "⚙️",
    color: "text-cyan-400",
  },
  "mid-pre-hard": {
    title: "Mid Pre-Hardmode",
    icon: "⚡",
...
function parseMarkdownFile(filePath, assetsDir, gameStage) {
  ...
  for (const line of lines) {
    ...
    // Check if this is a class header (no markdown formatting, just the class name)
    ...
    if (
      ...
      trimmedLine === "Ammunition" ||
      trimmedLine === "Support Tools" ||
      trimmedLine === "Sentries & Banners" ||
      trimmedLine === "Buffs"
    ) {
      if (currentClass) {
        if (trimmedLine === "Weapons") {
          currentSection = "weapons";
        } else if (trimmedLine === "Armors") {
          currentSection = "armor";
        } else if (trimmedLine === "Accessories") {
...
function parseItemsFromLine(line, assetsDir, gameStage, className, itemType) {
  const items = [];
  // Match pattern: [![ItemName.png](/images/path/path/ItemName.png?hash)](/wiki/ItemName "ItemName")[ItemName](/wiki/ItemName "ItemName")
  const regex =
    /\[\!\[([^.]+)\.png\]\(([^)]+)\)\]\([^)]+\)\[([^\]]+)\]\([^)]+\)/g;
  ...
  while ((match = regex.exec(line)) !== null) {
    ...
    items.push({
      name: itemName,
      icon: relativePath, // Use relative path
      link: `/wiki/${encodeURIComponent(itemName.replace(/'/g, "%27"))}`,
    });
  }

  // Also handle single item format without list markers
  if (items.length === 0) {
    const singleMatch = line.match(
      /\[\!\[([^.]+)\.png\]\(([^)]+)\)\]\([^)]+\)\[([^\]]+)\]\([^)]+\)/
    );
    if (singleMatch) {
      const iconFileName = singleMatch[1];
      const fullImagePath = singleMatch[2];
      const itemName = singleMatch[3];
...
async function main() {
  ...
  const outputPath = path.join(__dirname, "../public/thorium.json");
  const assetsDir = path.join(__dirname, "../public/thorium-assets");
  ...
  // Define the correct order of game stages
  const stageOrder = [
    "pre-boss",
    "pre-eater",
    "mid-pre-hard",
  ...
  for (const stageName of stageOrder) {
    const file = `thorium-${stageName}.md`;
