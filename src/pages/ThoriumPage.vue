<script setup lang="ts">
  ...
  import { thoriumGameStage, thoriumPlayerClass } from "@/stores/settings";
  ...
  const baseIconUrl = "/thorium-assets/";

  const loadData = async () => {
    try {
      // Load local thorium.json file
      const response = await ofetch<GameStage[]>("/thorium.json", {
  ...
  const currentGameStage = computed(() => data.value[thoriumGameStage.value]);
  const currentPlayerClass = computed(
    () => currentGameStage.value?.classes[thoriumPlayerClass.value]
  );
...
<template>
  <main class="min-h-screen bg-base-100 text-base-content flex">
    ...
    <div
      ...
      <GameStageSelector
        :game-stages="data"
        :selected-game-stage="thoriumGameStage"
        :base-icon-url="baseIconUrl"
        @update:selected-game-stage="thoriumGameStage = $event" />
      ...
      <div class="flex-1 flex flex-col lg:flex-row lg:overflow-hidden">
        ...
        <div
          ...
          :key="thoriumGameStage">
          <ClassSelector
            v-if="currentGameStage?.classes"
            :classes="currentGameStage.classes"
            :selected-player-class="thoriumPlayerClass"
            @update:selected-player-class="thoriumPlayerClass = $event" />
        </div>

        <!-- Content Layout -->
        <div
          class="content-container h-full w-full"
          :key="`${thoriumGameStage}-${thoriumPlayerClass}`">
          <div
